...
# (nombre, applies_to, rendimiento_diario) -> [(material_id, consumo, desperdicio)]
# material_id refiere a la tabla materials de la organización.
RECIPES: dict[tuple[str, str, float], list[tuple[int, float, float]]] = {
    ("Muro Ladrillo Hueco 18x18x33", "wall", 12.0): [
        (1, 16.0, 0.05),   # ladrillo hueco 18 (un/m²)
        (6, 0.20, 0.10),   # cemento (bolsa/m²)
        (7, 0.03, 0.10),   # arena (m³/m²)
        (13, 1.3, 0.0),    # mano de obra ayudante (hs/m²)
    ],
    ("Contrapiso y Carpeta", "room_floor", 40.0): [
        (6, 0.30, 0.10), (7, 0.09, 0.10), (13, 0.6, 0.0),
    ],
    ("Piso Porcelanato 60x60", "room_floor", 20.0): [
        (3, 1.0, 0.10), (6, 0.12, 0.10), (13, 0.9, 0.0),
    ],
    ("Columna H°A° 20x20", "column", 4.0): [
...
def seed(org_id: int, apply_to_plan: int | None) -> None:
    ...
    try:
        ...
        for (name, applies_to, yield_), lines in RECIPES.items():
